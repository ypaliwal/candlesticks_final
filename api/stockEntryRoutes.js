...
router.post('/newStock',function(req,res){
	...
	newStock.ownerId = req.body.ownerId;
...
// Get a stock by ownerId (probably email)
router.get('/stockByName/:ownerId', function(req, res) {
	StockEntry.find(function(err, objectsArray) {
		if (err) {
			console.log(err);
			res.status(400)
			...
			for(var i = 0; i < objectsArray.length; i++) {
				if(objectsArray[i].ownerId == req.params.ownerId) {
